...
## 五、 心得與結論
此次實驗證實演算法的「階數優劣」是決定效能的主因。當資料規模突破萬級後，選擇正確的演算法，其效能提升遠大於單純升級硬體。實測顯示，合併排序雖具備驚人的執行速度，但消耗 $O(n)$ 空間的特性也體現了「以空間換取時間」的權衡；而插入排序雖在大規模數據下較慢，但其原地排序與處理幾近排序數據的高效率，在特定低資源場景中仍具不可替代的價值。

對我而言，這份報告是一場充實的自我跨越。從最初對枯燥理論感到迷惘，到親手寫下代碼並看著數據在網站上即時呈現，那種將抽象邏輯轉化為實體網頁的過程，帶給我前所未有的成就感。這段旅程不僅讓我掌握了複雜度計算，更讓我體會到解決問題的樂趣與從無到有的創造喜悅，是近期收穫最豐富的一段成長歷程。
